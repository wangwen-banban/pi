...
The task list remains editable while the command runs. New user prompts can add, remove, or reprioritize work using the current plan revision. It is a current-goal view rather than permanent history: the model omits terminal or obsolete tasks once their outcomes no longer affect analysis, retry, verification, or decisions; active managed runs remain protected. Exit 0, non-zero exit, signal, wall timeout, explicit stop, or an opt-in fail-closed health/progress policy updates the linked task. Health-aware remote/adoption monitors report machine-readable heartbeat, availability, and opaque progress on a dedicated control fd; Pi never guesses remote semantics from log text. Terminal results enter a session-bound durable wake outbox. Plan, pending, and delivery markers must persist before send; each attempt carries exact run/sequence identities, and only a matching final `stop` response at a truly idle settle writes the explicit acknowledgement. Missing starts are retried with a watchdog and bounded backoff; `toolUse`, length, errors, and aborts never acknowledge. Completion waits for a fully settled agent and never interrupts an in-flight answer.
...
Privacy-minimal v3 terminal manifests live under `~/.pi/agent/background-task-runs/` with strict user-only permissions and are Git-ignored. No command metadata or stdout/stderr log files are created; output remains only in a bounded in-memory completion tail. Storage and session markers contain only safe session/run/task bindings, terminal classification, limited timing/health metadata, and delivery state—never command, cwd/path, title, output, credentials, context, PID, or start tokens. PI WEB Activity receives the same privacy-trimmed lifecycle projection. See [`extensions/background-tasks/README.md`](extensions/background-tasks/README.md).
